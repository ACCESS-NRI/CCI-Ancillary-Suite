...
#### Inputs
* `SEASALT_SOURCE`: Source sea salt aerosol climatology, from `${ANCIL_MASTER}/atmos/master/aero_clims/GA6.0_antie/v1/qrclim.sslt.nc`.
...
#### Outputs
* `OUTPUT`: Path to write the regridded sea salt aerosol climatology to, as a UM ancillary (via `ants.io.save.ancil`) plus a NetCDF copy (via `ants.io.save.netcdf`).
