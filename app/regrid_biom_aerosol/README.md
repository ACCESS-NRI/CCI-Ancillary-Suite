...
#### Inputs
* `BIOM_SOURCE`: Source biomass burning aerosol climatology, from `${ANCIL_MASTER}/atmos/master/aero_clims/GA6.0_antie/v1/qrclim.bio.nc`.
...
#### Outputs
* `OUTPUT`: Path to write the regridded biomass burning aerosol climatology to, as a UM ancillary (via `ants.io.save.ancil`) plus a NetCDF copy (via `ants.io.save.netcdf`).
