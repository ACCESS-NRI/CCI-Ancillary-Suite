...
#### Inputs
* `BLACK_CARBON_SOURCE`: Source black carbon aerosol climatology, from `${ANCIL_MASTER}/atmos/master/aero_clims/GA6.0_antie/v1/qrclim.blck.nc`.
...
#### Outputs
* `OUTPUT`: Path to write the regridded black carbon aerosol climatology to, as a UM ancillary (via `ants.io.save.ancil`) plus a NetCDF copy (via `ants.io.save.netcdf`).
