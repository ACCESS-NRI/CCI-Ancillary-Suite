...
#### Inputs
* `SULP_SOURCE`: Source sulphate aerosol climatology, from `${ANCIL_MASTER}/atmos/master/aero_clims/GA6.0_antie/v1/qrclim.sulp.nc`.
...
#### Outputs
* `OUTPUT`: Path to write the regridded sulphate aerosol climatology to, as a UM ancillary (via `ants.io.save.ancil`) plus a NetCDF copy (via `ants.io.save.netcdf`).
