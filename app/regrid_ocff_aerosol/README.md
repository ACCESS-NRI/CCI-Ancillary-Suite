...
#### Inputs
* `OCFF_SOURCE`: Source organic carbon from fossil fuel aerosol climatology, from `${ANCIL_MASTER}/atmos/master/aero_clims/GA6.0_antie/v1/qrclim.ocff.nc`.
...
#### Outputs
* `OUTPUT`: Path to write the regridded organic carbon from fossil fuel aerosol climatology to, as a UM ancillary (via `ants.io.save.ancil`) plus a NetCDF copy (via `ants.io.save.netcdf`).
